    ...
    <div dir="ltr" className="[unicode-bidi:plaintext]">
      ...
      <BasicPage>
        ...
        <CovenantSection
          ...
          rules={[
            { heading: "Short description", text: "This is your place to express what makes your server different from others. Do not describe what Mastodon is. Is it hosted by a specific organization? Is it operated from a specific country or region? Stick to one or two sentences. Attention is a limited resource. If your server's target audience speaks a different language than English, write the description in that language." },
            { heading: "Thumbnail", text: "Avoid the default. Express the visual identity of your server. A high quality image will make your server look professional. Stick close to the recommended dimensions of 1200x630px and use only JPG and PNG formats. Avoid transparency. We will proxy the image to avoid sending traffic from our visitors directly to your host." },
            { heading: "Language", text: "If you are running a language-specific server, make sure to set DEFAULT_LANGUAGE in your configuration to prevent Mastodon from showing the interface in the language of the visitor." },
            { heading: "Rules", text: "Make sure to create server rules — free-text rules in extended server description are not enough. Server rules are an integral part of onboarding. They must be kept short and to the point (you can elaborate in the extended server description). As a rule they should fit on a typical phone screen without having to scroll through. You want the visitor to be able to read them quickly. Do not include items that are covered by hardcoded report reasons, such as spam." },
            { heading: "Contact information", text: `Have a valid and reachable e-mail address for inquiries ("business e-mail"). We may contact you through that address. And in cases of legal matters, it is generally preferable to receive e-mails through your own address rather than through your web host's abuse contact. You shoud also have a contact account ("Contact username") though it does not have to be your personal account.` },
          ]}
        />
...
const CovenantSection = ({ heading, description, rules }: { heading?: string; description?: React.ReactNode | string; rules?: { heading: string; text: string; }[] }) => {
  return (
    <section>
