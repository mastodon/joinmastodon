...
const fetchJson = async (url) => {
  const res  = await fetch(url, { headers: { 'Authorization': `Bearer ${process.env.PATREON_ACCESS_TOKEN}` } })
...
while (true) {
  ...
  data.data.forEach(member => {
    ...
    const currentlyEntitledTiers = member.relationships.currently_entitled_tiers.data
...
const silver = membersByTiers["Silver sponsor"].map(member => ({ url: member.note, name: member.name, logo: member.picture, nofollow: true, date: member.joinedAt })).concat(membersByTiers["Silver sponsor (grandfathered)"].map(member => ({ url: member.note, name: member.name, logo: member.picture, nofollow: false, date: member.joinedAt }))).sort((a, b) => (new Date(a.date)) - (new Date(b.date))).map(member => ({ url: member.url, logo: member.logo, name: member.name, url: member.url, nofollow: member.nofollow }))
const generalHighlighted = membersByTiers["Highlighted sponsor"].sort((a, b) => b.lifetimeSupportCents - a.lifetimeSupportCents).map(member => member.name)
const general = membersByTiers["Sponsor"].sort((a, b) => b.lifetimeSupportCents - a.lifetimeSupportCents).map(member => member.name)

fs.writeFile("./data/patreon.json", JSON.stringify({
  silver,
  generalHighlighted,
  general,
}, null, "  "), err => {
